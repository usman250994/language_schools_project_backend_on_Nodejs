...
class ClassroomRepo {
  ...
  async create(school: School, division: Division, name: string, section: string): Promise<Classroom> {
  ...
  async getAllBySchoolId(schoolId: string, offset: number, limit: number, name = ''): Promise<[Classroom[], number]> {
    ...
    return query
      ...
      .leftJoinAndSelect('division_classroom', 'division_classroom', 'division_classroom.classroomId = classroom.id')
      .leftJoinAndMapMany('classroom.division', 'division', 'divisions', 'divisions.id = division_classroom.divisionId')
      .take(limit).skip(offset).getManyAndCount();
  }

  async addToUser(classroom: Classroom, division: Division, user: User): Promise<void> {
    const relation = await this.classroomUserRepo.findOne({ user });

    if (relation) {
      await this.classroomUserRepo.update({ userId: relation.userId }, { classroomId: classroom.id, divisionId: division.id });

      return;
    }

    console.log({ user, classroom, division });
    await this.classroomUserRepo.save({ user, classroom, division });
