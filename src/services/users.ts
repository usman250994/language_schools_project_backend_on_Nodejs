...
async function setupStudent(student: User, { classRoomId, parentId }: { classRoomId: string; parentId: string }): Promise<User>{
    if (classRoomId) {
        ...
        await ClassroomRepo.addToUser(classroom, student);
    }
...
export async function createStudent(student: Partial<User>, { classRoomId, parentId }: { classRoomId: string; parentId: string }): Promise<User> {
    const _student = await UserRepo.createStudent(student);

    return setupStudent(_student, { classRoomId, parentId });
}

export async function updateStudent(student: Partial<User>, { classRoomId, parentId }: { classRoomId: string; parentId: string }, studentId: string): Promise<User> {
    ...
    return setupStudent({ ..._student, ...student }, { classRoomId, parentId });
}
