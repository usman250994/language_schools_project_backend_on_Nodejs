...
export async function getAttendance(classroomId: string): Promise<{ timeTable: TimeTable; students: User[] }> {
    const classroom = await findClassroomById(classroomId);
    const timeTable = await TimeTableRepo.findByProp({ classroom });
...
export async function markClassAttendance(classroomId: string, statuses: StudentAttendanceStatus[]): Promise<void> {
    const classroom = await findClassroomById(classroomId);
