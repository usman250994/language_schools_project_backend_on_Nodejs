...
import {
    ...
    getUsersByRoleAndClass, updateClassroom,
...
router.put('/classrooms/:classroomId/schools/:schoolId', authorize([Role.ADMIN, Role.TEACHER, Role.PARENT]), wrapAsync(async (req: Request, res: express.Response) => {
    if (!isUserReq(req)) {
        throw new Error('User not found in session');
    }
